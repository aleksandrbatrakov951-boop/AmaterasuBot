...
async def notify_admin(update: Update, context: ContextTypes.DEFAULT_TYPE, question_id: str, user, question_text: str, admin_id: int):
    """Уведомление конкретного администратора - БЕЗ MARKDOWN"""
    
    keyboard = [[InlineKeyboardButton("📝 Ответить", callback_data=f"answer_{question_id}")]]
    reply_markup = InlineKeyboardMarkup(keyboard)
    
    # ПРОСТОЙ текст без Markdown - экранируем ВСЕ специальные символы
    safe_question_id = escape_text(question_id)
    safe_first_name = escape_text(user.first_name)
    safe_username = escape_text(user.username or 'нет')
    safe_question_text = escape_text(question_text)
    
    message_text = (
        f"🆕 НОВЫЙ ВОПРОС #{safe_question_id}\n\n"
        f"👤 От пользователя: {safe_first_name}\n"
    ...
    try:
        ...
        await context.bot.send_message(
            ...
            text=message_text,
            reply_markup=reply_markup,
            parse_mode='MarkdownV2'  # Используем MarkdownV2 с экранированным текстом
        )
