...
ADMINS = {
    8004182708: {"name": "Пряник", "username": "Deluse_SC"},
    7725062904: {"name": "Нефи", "username": "voidNameFame"}
}
...
class QuestionBot:
    ...
    def get_admin_questions(self, admin_id):
        """Возвращает вопросы для конкретного администратора"""
        return [q for q_id, q in self.questions.items() if q['admin_id'] == admin_id]
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "choose_admin":
        ...
        question = question_bot.get_question_by_id(question_id)
    ...
    if context.user_data.get('awaiting_question') and context.user_data.get('selected_admin'):
        ...
        if question:
            ...
            try:
                ...
                admin_info = ADMINS[update.effective_user.id]
                await update.message.reply_text(
                    f"📨 Ответ от {admin_info['name']} успешно доставлен пользователю!"
                )
...
async def admin_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    admin_questions = question_bot.get_admin_questions(user_id)
    ...
    stats_text = (
        ...
        f"📨 Ожидают ответа: {new_questions}\n"
    ...
    keyboard = []
    if new_questions > 0:
        keyboard.append([InlineKeyboardButton("📨 Ответить на новые вопросы", callback_data="choose_admin")])
    keyboard.append([InlineKeyboardButton("📈 Общая статистика", callback_data="stats")])
...
async def debug_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    debug_info = (
        ...
        f"📁 Questions file: {'✅ EXISTS' if os.path.exists(QUESTIONS_FILE) else '❌ MISSING'}\n"
        f"📊 Total questions: {len(question_bot.questions)}\n"
        f"👥 Admins configured: {len(ADMINS)}\n"
        f"🔄 Health check: ✅ Active"
    )
