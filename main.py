...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Обработка оценок
        ...
        # Если это подробная оценка (новые быстрые реакции)
        if len(parts) == 3 and parts[2] == "detailed":
            # Показываем обычные звезды для подробной оценки
            rating_keyboard = [
                [
            ...
            await query.edit_message_text(
                "💫 Оцените качество помощи администратора:",
                reply_markup=InlineKeyboardMarkup(rating_keyboard)
            )
        
        # Старая система оценок (оставляем для совместимости)
    ...
    # Администратор отвечает на вопрос
        ...
        if question_id in question_bot.questions:
            ...
            try:
                ...
                # Предлагаем оценить ответ с НОВЫМИ быстрыми реакциями
...
async def ask_for_rating(context, question_id, user_id):
    """Предлагает пользователю оценить ответ с НОВЫМИ быстрыми реакциями"""
    reaction_keyboard = [
        [
    ...
    try:
        ...
        print(f"✅ Отправлены быстрые реакции для вопроса #{question_id}")
...
# ==================== ФОНТАСКОВАЯ ЗАДАЧА ДЛЯ НАПОМИНАНИЙ ====================
async def reminder_background_task(application):
    """Фоновая задача для проверки напоминаний"""
    while True:
        try:
            await reminder_system.check_pending_questions(application)
...
def main():
    ...
    print(f"✅ Загружено предпочтений: {len(user_preferences.preferences)} пользователей")
    print("=" * 60)
    ...
    try:
        ...
        application.job_queue.run_repeating(
            lambda context: asyncio.create_task(reminder_system.check_pending_questions(context.application)),
